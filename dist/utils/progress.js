...
export function displayDownloadStatus(status) {
    const statusColors = {
        queued: chalk.yellow,
    ...
    const progressBar = status.status === 'downloading'
        ...
        : '';
    console.log(`  ${color(status.status.padEnd(12))} ${chalk.white(status.title.substring(0, 40))} ${progressBar}`);
    if (status.error) {
...
export function displayStatusReport(response) {
