...
export async function searchCommand(query, options) {
    ...
    try {
        ...
        if (options.audiobook) {
            filteredResults = filteredResults.filter(r => AUDIOBOOK_FORMATS.includes(r.format?.toLowerCase() || ''));
        ...
        if (options.ebook) {
            filteredResults = filteredResults.filter(r => EBOOK_FORMATS.includes(r.format?.toLowerCase() || ''));
        }
        if (options.format) {
            filteredResults = filteredResults.filter(r => r.format?.toLowerCase() === options.format.toLowerCase());
        }
